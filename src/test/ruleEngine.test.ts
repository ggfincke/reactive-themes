...
suite("Rule Engine", () => {
    test("matches common glob features (braces, negation, matchBase)", () => {
        assert.ok(
            matchGlobPattern("/workspace/src/index.ts", "*.ts"),
            "matchBase should match *.ts"
        );
        assert.ok(matchGlobPattern("/workspace/src/button.test.ts", "**/*.test.{ts,js}"));
        assert.strictEqual(
            matchGlobPattern("/workspace/node_modules/pkg/index.js", "!**/node_modules/**"),
            false
        );
        assert.ok(
            matchGlobPattern("C:/dev/project/src/App.vue".replace(/\\/g, "/"), "**/*.{ts,js,vue}")
        );
    });
